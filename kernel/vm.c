...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    ...
    if(do_free){
      ...
      remove_from_frame_table(pagetable, a);
      kfree((void*)pa);
    }
    *pte = 0;
  }
...
vmfault(pagetable_t pagetable, uint64 va, int read)
{
  ...
  add_to_frame_table((uint64)mem, p, va);
  return mem;
}
...
void add_to_frame_table(uint64 pa, struct proc *p, uint64 va) {
  acquire(&ftable_lock);
retry:
  for(int i = 0; i < MAX_FRAMES; i++) {
    if(frames_table[i].allocated == 0) {
  ...
  void *evicted_page = evict_page();
  if(evicted_page == 0) {
    panic("add_to_frame_table: no frames and eviction failed");
  }
  kfree(evicted_page);
  acquire(&ftable_lock);
